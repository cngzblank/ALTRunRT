...
/// Get the data directory (next to the executable, like original ALTRun)
...
pub fn shortcut_list_path() -> PathBuf {
    data_dir().join("ShortCutList.txt")
}

pub fn config_path() -> PathBuf {
    data_dir().join("ALTRun.ini")
}
...
fn default_shortcut_list() -> Vec<ShortCutItem> {
    let defaults = vec![
        ...
        ("msconfig", "System Config", "msconfig", ParamType::None, 5),
        ("control", "Control Panel", "control.exe", ParamType::None, 5),
        ("appwiz", "Add/Remove Programs", "appwiz.cpl", ParamType::None, 5),
        ("devmgmt", "Device Manager", "devmgmt.msc", ParamType::None, 5),
